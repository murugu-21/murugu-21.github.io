...
describe("path versioning", () => {
  const endpoints = ["/profile", "/experience", "/skills", "/education", "/open-source", "/posts"];
  ...
  it("serves the same body under the unversioned alias", async () => {
    for (const endpoint of endpoints) {
      const versioned = await (await get(`/api/${CURRENT_API_VERSION}${endpoint}`)).text();
...
describe("rate-limit headers", () => {
  ...
  it("counts down across requests from the same client", async () => {
    ...
    expect(res.headers.get("X-RateLimit-Remaining")).toBe(String(READ_QUOTA.quota - 2));
  });
  ...
  it("reports the allowance a dry run did not spend", async () => {
    ...
    expect(first.headers.get("X-RateLimit-Remaining")).toBe(String(CONTACT_DAILY_PER_CLIENT));
