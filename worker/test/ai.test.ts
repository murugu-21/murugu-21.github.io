...
describe("runDeepseekExchange", () => {
  ...
  it("throws on a non-ok response", async () => {
    const fetcher = (async () => new Response("nope", { status: 402 })) as typeof fetch;
    await expect(runDeepseekExchange("sk-test", [], () => {}, fetcher)).rejects.toThrow(
      "deepseek request failed: 402"
    );
  });
});
...
describe("isInsufficientBalance", () => {
  it("recognises a 402 from an exchange and nothing else", async () => {
    ...
    const err = await runDeepseekExchange("sk-test", [], () => {}, failing(402)).catch(
      (e: unknown) => e
    );
    expect(isInsufficientBalance(err)).toBe(true);

    const other = await runDeepseekExchange("sk-test", [], () => {}, failing(500)).catch(
      (e: unknown) => e
    );
